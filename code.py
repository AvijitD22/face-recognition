...
while True:
    ...
    try:
        ...
        if confidence > 85:
            cv2.putText(image, "Hey Nacon", (250, 450), cv2.FONT_HERSHEY_COMPLEX, 1, (0,255,0), 2)
            cv2.imshow('Face Recognition', image )
            # phn = phone Number , msg = message
            pywhatkit.sendwhatmsg_instantly(phone_no=phn,
                                 message=msg)
            ...
            1
            os.system("aws ec2 run-instances  --image-id (ami ID) --instance-type (instance type)  --subnet-id (subnet ID)  --count 1 --security-group-ids (Security Group ID)   --key-name (Key Name)  > ec2.txt")
